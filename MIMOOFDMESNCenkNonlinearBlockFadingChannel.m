...
IsiDuration  = 6;     % Number of multipath components  
EbNoDB = -0:5:30;          % Signal-to-noise ratio
%%%%%%%%%%%%%%%%%%%%%% MIMO Parameters %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
m = 4;                % Data symbols QAM Modulation Order
m_pilot = 4;          % Pilot Symbols Modulation Order
NumOfdmSymbols = 200; % Number of OFDM symbols to simulate for the BER curve 
...
nInternalUnits = 50;
inputScaler = 0.000001;
inputOffset = 0.0;
feedbackScaler = 0.001;
teacherScaling = 0.000015.*ones(N_t*2,1);
spectralRadius = 0.98;
% during training. 
% Secondary parameters
...
for jj = 1:length(EbNoDB)
    A_Clip = sqrt(var_x(jj)).*10.^(ClipLeveldB./20);
    ...
    for kk = 1:NumOfdmSymbols 
        ...
        if(mod(kk,L) == 1)
            ...
            x_CP_NLD = x_CP./((1+(abs(x_CP)./A_Clip).^(2.*p_smooth)).^(1./(2.*p_smooth)));
            y_CP_NLD = zeros(N+CyclicPrefixLen,N_r);
            for nnn = 1:N_r
                    % Superimpose all transmitted and filtered streams to form
                    % the received time domain signal in each receive antenna. 
                    for mmm = 1:N_t
                        y_CP_NLD(:,nnn) = y_CP_NLD(:,nnn) + filter(c{nnn}{mmm},1,x_CP_NLD(:,mmm));
                    end
            ...
            % Train the ESN
            [trainedEsn, Delay, Delay_Min, Delay_Max, nForgetPoints, NMSE_ESN] = trainMIMOESN(esn, DelayFlag, Min_Delay, Max_Delay, ...
                      CyclicPrefixLen, N, N_t, N_r, IsiDuration, y_CP_NLD, x_CP);  
            ...
            for nnn = 1:N_r
                    ...
                    y_CP_NLD(:,nnn) = y_CP_NLD(:,nnn) + sqrt(size(y_CP,1)*No/2)*randn(size(y_CP,1),2)*[1;j];
                    Y_NLD(:,nnn) = 1/N*fft(y_CP_NLD(IsiDuration:end,nnn));
            end             
            ...
            ESN_input(:,1) = [real(y_CP_NLD(:,1)); zeros(Delay_Max,1)];
            ESN_input(:,2) = [imag(y_CP_NLD(:,1)); zeros(Delay_Max,1)];
            ESN_input(:,3) = [real(y_CP_NLD(:,2)); zeros(Delay_Max,1)];
            ESN_input(:,4) = [imag(y_CP_NLD(:,2)); zeros(Delay_Max,1)];
